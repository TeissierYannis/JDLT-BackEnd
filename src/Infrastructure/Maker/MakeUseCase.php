...
class MakeUseCase extends AbstractMaker
{
    ...
    public function generate(InputInterface $input, ConsoleStyle $io, Generator $generator)
    {
        ...
        $useCaseClassNameDetails = $generator->createClassNameDetails(
            $input->getArgument('name'),
            sprintf('Domain\\%s\\UseCase', Str::asCamelCase($input->getArgument("domain"))),
            ''
        );

        $requestClassNameDetails = $generator->createClassNameDetails(
            $input->getArgument('name'),
            sprintf('Domain\\%s\\Request', Str::asCamelCase($input->getArgument("domain"))),
            'Request'
        );

        $responseClassNameDetails = $generator->createClassNameDetails(
            $input->getArgument('name'),
            sprintf('Domain\\%s\\Response', Str::asCamelCase($input->getArgument("domain"))),
            'Response'
        );

        $presenterClassNameDetails = $generator->createClassNameDetails(
            $input->getArgument('name'),
            sprintf('Domain\\%s\\Presenter', Str::asCamelCase($input->getArgument("domain"))),
            'PresenterInterface'
        );
